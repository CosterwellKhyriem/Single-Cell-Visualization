                       ...
                       lg.pos="right",border=F,alpha=0.8,pt.border.size = 0.1)
{
    ...
    p <- p + theme(legend.key.size = unit(5, 'cm'), #change legend key size
...
customUmap(umap_emb)
```
We can fix the cluster color by either choosing our own color for each cluster.<br>
or, by saving the randomly generated color into a vector.<br>
...
Another beautification that we can include is to add borders around the points to make them look 3D.<br>
Borders can be added by using the border=T option and the size of the border can be controlled by using the pt.broder.size option. <br>
...
However, in point plots like the UMAP, points tends to get overlay over each other.
This may undermind or over-estimate the real expression pattern in a cluster.

Here, we will explore a package called Nebulosa which uses kernal density to show the distribution of expression valies in a UMAP plot.
